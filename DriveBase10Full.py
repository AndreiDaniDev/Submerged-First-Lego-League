...
class DriveBase():
    ...
    async def gyroForward(self, distance: float, speed: int, *, kp: float, ki: float, kd: float, dt: int, thresholdIntegral: int, thresholdStallDetection = 10, iteratorStallDetection: int = 5, stop: bool = True) -> None:
        ...
        '''
            ...
            stop - tells the robot if we want to stop it after it has travelled the given distasnce \n
        ...
        kp = abs(kp); ki = abs(ki); kd = abs(kd); kp = abs(kp); dt = abs(dt); distance = abs(distance); speed = abs(speed)
        ...
        while(self.position < self.reachDistance):
            ...
            # self.correction = DriveBase.limitAbs(self, self.correction, self.plusCorrection, self.minusCorrection)
    ...
    async def gyroBackwards(self, distance: float, speed: int, *, kp: float, ki: float, kd: float, dt: int, thresholdIntegral: int, thresholdStallDetection = 10, iteratorStallDetection: int = 5, stop: bool = True) -> None:
        # ---> Needed to know <------> Documentation <---
    ...
    async def turnLeft(self, degrees: int, speedMax: int, speedMin: int, *, kp: float = 0.25, dt: int = 2, kLeft: float = -1, kRight: float = 1, thresholdError: int = 5, thresholdStallDetection = 10, iteratorStallDetection: int = 5, stop: bool = True) -> None:
        # ---> Needed to know <------> Documentation <---
        ...
        '''
            ...
            kp, dt, kLeft, kRight - constants used for the Proportional Controller + applying the speeds \n
            kLeft, kRight - should be set to one of these combinations: [[-1, 1], [-1, 0], [0, 1]] \n
            kLeft, kRight - but if you want to experiment they can be any value in the range of [-1, 1] \n
        ...
        # ---> Initialization <--- + ---> Error Checking <---
        self.iterator = 0; gs.reset_yaw(gs.tilt_angles()[0] - self.reachTarget); self.reachTarget = -int(10 * abs(degrees)); kp = abs(kp)
        ...
        while(self.error < -thresholdError or self.error > thresholdError):
            ...
            self.speedLeft = int(DriveBase.limitAbs(self, self.proportional * kLeft, speedMax, speedMin))
            self.speedRight = int(DriveBase.limitAbs(self, self.proportional * kRight, speedMax, speedMin))
    ...
    async def turnRight(self, degrees: int, speedMax: int, speedMin: int, *, kp: float = 0.25, dt: int = 2, kLeft: float = 1, kRight: float = -1, thresholdError: int = 5, thresholdStallDetection = 10, iteratorStallDetection: int = 5, stop: bool = True) -> None:
        # ---> Needed to know <------> Documentation <---
        ...
        '''
            ...
            speedMax, speedMin - two pointers that make the speed applied always be bounded between these two \n
            kp, dt, kLeft, kRight - constants used for the Proportional Controller + applying the speeds \n
            kLeft, kRight - should be set to one of these combinations: [[1, -1], [0, -1], [1, 0]] \n
            kLeft, kRight - but if you want to experiment they can be any value in the range of [-1, 1] \n
            thresholdError - the max error that is accepted when turning (must be >= 5) \n
            thresholdStallDetection - the value that is used to tell if the robot is stuck or not \n
            iteratorStallDetection - how frequent we check if the robot is stuck (affected by dt) \n
