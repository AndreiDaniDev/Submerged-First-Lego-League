...
class DriveBase():
    ...
    async def gyroForward(self, distance: float, speed: int, *, kp: float, ki: float, kd: float, dt: int, thresholdIntegral: int, thresholdStallDetection = 10, iteratorStallDetection: int = 5, stop: bool = True) -> None:
        ...
        # if(kp == 0): print("Error - Kp = 0"); return None
    ...
    async def turnLeft(self, degrees: int, speedMax: int, speedMin: int, *, kp: float, dt: int, kLeft: float = -1, kRight: float = 1, thresholdError: int = 5, thresholdStallDetection = 10, iteratorStallDetection: int = 5, stop: bool = True) -> None:
        # ---> Needed to know <------> Documentation <---
        ...
        while(self.error < -thresholdError or self.error > thresholdError):
            ...
            self.speedLeft = int(DriveBase.limitAbs(self, self.proportional * kLeft, speedMax, speedMin)) * (kLeft != 0)
            self.speedRight = int(DriveBase.limitAbs(self, self.proportional * kRight, speedMax, speedMin)) * (kRight != 0)
