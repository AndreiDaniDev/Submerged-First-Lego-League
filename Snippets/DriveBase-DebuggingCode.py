...
class DriveBase(object):
    ...
    def PID_Controller(self, error: int, integralLimit: int, sign: int, scaleConstants: int) -> None:
        ...
        print('E=' + str(self.error), end = ' ')
        print('P=' + str(self.proportional), end = ' ')
        print('I=' + str(self.integral), end = ' ')
        print('D=' + str(self.derivative), end = ' ')
        print('C=' + str(self.controllerOutput), end = ' ')
    ...
    async def gyroForwards(self, distance: int, lowestSpeed: int, highestSpeed: int, *, kp: int = 50, ki: int = 20, kd: int = 75, dt: int = 1, constantsScale: int = 100, integralLimit: int = 25000, stallDetectionIterator: int = 2000, stallDetectionThreshold: int = 5, accelerationScale: int = 100, easingMethod = SpeedMethods.easeOutInQuad, stop: bool = True, brakeMethod = motor.SMART_BRAKE) -> None:
        ...
        # For debugging purposes only
        print("DriveBase-Gyro-Forwards " + "Dist = " + str(distance) + " Speeds = (" + str(lowestSpeed) + ", " + str(highestSpeed) + ")", end = ' ')
        print("Kp=" + str(self.kp), end = ' ')
        print("Ki=" + str(self.ki), end = ' ')
        print("Kd=" + str(self.kd), end = ' ')
        print("Scale=" + str(constantsScale), end = ' ')
        print("E=" + str(self.reachAngle - self.angle), end = ' | ')
        ...
        while(self.position < self.reachDistance):
            ...
            print('S=' + str(self.speed), end = ' | ') # For debugging purposes only
    ...
    async def gyroBackwards(self, distance: int, lowestSpeed: int, highestSpeed: int, *, kp: int = 50, ki: int = 20, kd: int = 75, dt: int = 1, constantsScale: int = 100, integralLimit: int = 25000, stallDetectionIterator: int = 2000, stallDetectionThreshold: int = 5, accelerationScale: int = 100, easingMethod = SpeedMethods.easeOutInQuad, stop: bool = True, brakeMethod = motor.SMART_BRAKE) -> None:
        ...
        # For debugging purposes only
        print("DriveBase-Gyro-Backwards " + "Dist = " + str(distance) + " Speeds = (" + str(lowestSpeed) + ", " + str(highestSpeed) + ")", end = ' ')
        print("Kp=" + str(self.kp), end = ' ')
        print("Ki=" + str(self.ki), end = ' ')
        print("Kd=" + str(self.kd), end = ' ')
        print("Scale=" + str(constantsScale), end = ' ')
...
class Programs(object):
    ...
    async def coralRun(self) -> None:
        driveBase.initRun()
        # ---> Code <---

        return None

    async def debuggingRun(self) -> None:
        driveBase.initRun()
        # ---> Code for debugging <---
        await driveBase.gyroForwards(500, 500, 500, kp = 160, ki = 10, kd = 25, constantsScale = 1000, easingMethod = SpeedMethods.easeOutInQuad)
        return None
...
class ProgramManager(Programs):
    def __init__(self) -> None:
        self.saveFile = fileHandeler('lastprogram.txt')
        self.programIDX: int = 1; self.idxMax: int = 8
        self.delay: int = 200
    ...
    async def PlayProgram(self, number: int) -> None:
        if(number == 0): await self.debuggingRun(); return None
    ...
    async def switchPrograms(self) -> None:
        ...
        while(1): # Select Programs
            ...
            if(self.getUserResponseLF()):
                ...
                self.saveFile.writeToFile(self.programIDX)
...
async def main() -> None:
    ...
    await programManager.PlayProgram(0)
    return None
