...
class SpeedMethods(object):
    ...
    '''
    # ---> To add costom functions you can use the following template <---
    @staticmethod
    def myFunction(x: float) -> float:
        return x; # Add your own formula and name
        # Be aware that x will always be in the range of [0, 1]
    '''

# ---> The class for the lower part of the robot that is responsible for moving <---
class DriveBase(object):

    def __init__(self, leftMotor: int, rightMotor: int, oneUnit: int, scale: int, brakeMethod) -> None:
        # ---> One unit in your system is equal to (oneUnit / ocale) degrees <---
    ...
    async def arcTurnForwards(self, radius: int) -> None:
        # radius in centimeters
        return None

    async def turnLeft(self, angle: int, startSpeed: int, endSpeed: int, *, kLeft: float = -1, kRight: float = 1, stallDetectionIterator: int = 2000, stallDetectionThreshold: int = 5, accelerationScale: int = 100, accelerationMethod = SpeedMethods.easeOutInQuad, addition: int = 0, divizor: int = 1, stop: bool = True):
    ...
    async def turnRight(self, angle: int, startSpeed: int, endSpeed: int, *, kLeft: float = 1, kRight: float = -1, stallDetectionIterator: int = 2000, stallDetectionThreshold: int = 5, accelerationScale: int = 100, accelerationMethod = SpeedMethods.easeOutInQuad, addition: int = 0, divizor: int = 1, stop: bool = True):
...
class Programs(object):
    def __init__(self) -> None:
        self.driveBase = DriveBase(hub.port.B, hub.port.D, 2045, 1000, motor.SMART_BRAKE)
    async def Run1(self) -> None:
        self.driveBase.initRun()
        await self.driveBase.gyroForwards(100, 200, 500, kp = 0.4, ki = 0.75, kd = 2)
        await self.driveBase.gyroBackwards(100, 200, 500, kp = 0.4, ki = 0.75, kd = 2)

# ---> The main program <---
async def main():
    driveBase = DriveBase(hub.port.B, hub.port.D, 2045, 1000, motor.SMART_BRAKE); driveBase.initRun()
    # ---> Approach 2 to implementing runs (SEPERATE PROGRAMS) (WORST HAND SCENARIO) <---
    '''
    # ---> Standard Test for moving forwards & backwards <---
    await driveBase.gyroForwards(100, 200, 500, accelerationMethod = SpeedMethods.easeOutInQuad)
    await driveBase.gyroBackwards(100, 200, 500, accelerationMethod = SpeedMethods.easeOutInQuad)
    '''
