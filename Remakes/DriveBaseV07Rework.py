...
class DriveBase(object):
    ...
    async def lineFollower(self, speed: int, colorSensorPort: int, *, blackValue: int = 50, kp: int = 50, ki: int = 20, kd: int = 75, dt: int = 1, constantsScale: int = 100, integralLimit: int = 250) -> None:

        '''
        ---> Parameter lineFollower <---
        speed: The speed that the robot will have along the route
        colorSensorPort: the port of the color sensor that will be over the
        blackValue: the reflection value (100 - true black, 0 - white)
        ...
        if(colorSensorPort != self.leftColorSensor and colorSensorPort != self.rightColorSensor):
            print("Error - Invalid Color Sensor Port"); return None

        otherSensorPort: int = 0 # The port of the other color sensor
        if(colorSensorPort == self.leftColorSensor): otherSensorPort = self.rightColorSensor
        if(colorSensorPort == self.rightColorSensor): otherSensorPort = self.leftColorSensor

        self.kp = abs(kp); self.ki = abs(ki); self.kd = abs(kd); self.dt = abs(dt)

        # Normalize the Speed Value
        if(speed > self.pairMaxSpeed): speed = self.usualMaxSpeed
        if(speed < self.pairMinSpeed): speed = self.usualMinSpeed
        self.speed = speed

        # self.leftColorError, self.rightColorError - Figurative Sense
        # self.leftColorError - the error of the color sensor that is on the line follower
        # self.rightColorError - the error of the other color sensor
        self.leftColorError = color_sensor.reflection(colorSensorPort) - blackValue
        self.rightColorError = color_sensor.reflection(otherSensorPort) - blackValue

        while(self.rightColorError > -25): # Run until the other color sensor detects a line
            self.leftColorError = color_sensor.reflection(colorSensorPort) - blackValue
            self.rightColorError = color_sensor.reflection(otherSensorPort) - blackValue
            self.PID_Controller(self.leftColorError, integralLimit, 1, constantsScale)
            self.leftSpeed = int(self.limit(self.speed + self.controllerOutput, self.pairMaxSpeed, self.pairMinSpeed))
            self.rightSpeed = int(self.limit(self.speed - self.controllerOutput, self.pairMaxSpeed, self.pairMinSpeed))
            motor_pair.move_tank(self.pair, self.leftSpeed, self.rightSpeed)
            await runloop.sleep_ms(self.dt)

        motor_pair.stop(self.pair, stop = self.brake)
        return None

    async def lineFollowerSimple(self, speed: int, *, blackValue: int = 50) -> None:
        while(1):
            if(color_sensor.reflection(self.rightColorSensor) < blackValue):
                motor_pair.move_tank(self.pair, speed, 0)
            else: motor_pair.move_tank(self.pair, 0, speed)
        return None
...
class Programs(object):
    ...
    async def Run1(self) -> None:
        driveBase.initRun()
        await driveBase.gyroForwards(2000, 200, 1000, kp = 4, ki = 2, kd = 2, constantsScale = 10)
        await driveBase.gyroBackwards(1500, 200, 1000, kp = 4, ki = 2, kd = 2, constantsScale = 10)
        return None

    async def Run2(self) -> None:
        driveBase.initRun()
        # ---> Standard Test for turns on the shortest path <---
        await driveBase.turnLeft(900, 200, 500, kLeft = -1, kRight = 1, error = 75, accelerationMethod = SpeedMethods.easeOutInQuad)
        await runloop.sleep_ms(1000)
        await driveBase.turnRight(900, 200, 500, kLeft = 1, kRight = -1, error = 75, accelerationMethod = SpeedMethods.easeOutInQuad)
    ...
    async def Run4(self) -> None:
        driveBase.initRun()
        print(color_sensor.reflection(driveBase.leftColorSensor))
        print(color_sensor.reflection(driveBase.rightColorSensor))

        # await driveBase.lineFollower(250, driveBase.rightColorSensor)
        await driveBase.lineFollowerSimple(250)
        return None
