...
class DriveBase(object):
    ...
    async def gyroForwards(self, distance: float, startSpeed: int, endSpeed: int, *, kp: float = 0.5, ki: float = 0.2, kd: float = 0.75, dt: int = 1, integralLimit: int = 250, stallDetectionIterator: int = 1000, stallDetectionThreshold: int = 5, accelerationScale: int = 100, accelerationMethod = SpeedMethods.liniar, stop: bool = True) -> None:
    ...
    async def gyroBackwards(self, distance: float, startSpeed: int, endSpeed: int, *, kp: float = 0.5, ki: float = 0.2, kd: float = 0.75, dt: int = 1, integralLimit: int = 250, stallDetectionIterator: int = 1000, stallDetectionThreshold: int = 5, accelerationScale: int = 100, accelerationMethod = SpeedMethods.liniar, stop: bool = True) -> None:
    ...
    async def turnLeft(self, angle: float, startSpeed: int, endSpeed: int, *, kLeft: float = -1, kRight: float = 1, stallDetectionIterator: int = 1000, stallDetectionThreshold: int = 2, accelerationScale: int = 100, accelerationMethod = SpeedMethods.liniar, addition: int = 0, divizor: int = 1, stop: bool = True):
        ...
        if((kLeft > 0 and kRight == 0) or (kLeft > 0 and kRight < 0) or (kLeft == 0 and kRight < 0)):
            ...
            await self.turnRight((360 - angle) / 3, self.startSpeed, self.endSpeed, kLeft = kLeft, kRight = kRight, stallDetectionIterator = stallDetectionIterator, stallDetectionThreshold = stallDetectionThreshold, accelerationScale = accelerationScale, accelerationMethod = accelerationMethod, addition = 0, divizor = 3, stop = True)
            await runloop.sleep_ms(1000)
            await self.turnRight((360 - angle) / 3, self.startSpeed, self.endSpeed, kLeft = kLeft, kRight = kRight, stallDetectionIterator = stallDetectionIterator, stallDetectionThreshold = stallDetectionThreshold, accelerationScale = accelerationScale, accelerationMethod = accelerationMethod, addition = 1, divizor = 3, stop = True)
            await runloop.sleep_ms(1000)
            await self.turnRight((360 - angle) / 3, self.startSpeed, self.endSpeed, kLeft = kLeft, kRight = kRight, stallDetectionIterator = stallDetectionIterator, stallDetectionThreshold = stallDetectionThreshold, accelerationScale = accelerationScale, accelerationMethod = accelerationMethod, addition = 2, divizor = 3, stop = stop)
            self.previousFunction = 3; return None

        gs.reset_yaw(gs.tilt_angles()[0] - self.reachAngle)
        self.angle = gs.tilt_angles()[0]; self.iterator = 0
        self.reachAngle = -(math.floor(10 * angle))
        ...
        self.wasStuck = False; self.kLeft = kLeft; self.kRight = kRight
        ...
        while(self.angle > self.reachAngle + 5):
            ...
            self.speed = self.startSpeed + self.getSpeed(accelerationMethod, addition, divizor)
        ...
        print("Turn Left Complete", addition)

        return None

    async def turnRight(self, angle: float, startSpeed: int, endSpeed: int, *, kLeft: float = -1, kRight: float = 1, stallDetectionIterator: int = 1000, stallDetectionThreshold: int = 2, accelerationScale: int = 100, accelerationMethod = SpeedMethods.liniar, addition: int = 0, divizor: int = 1, stop: bool = True):
        ...
        if((kLeft < 0 and kRight == 0) or (kLeft < 0 and kRight > 0) or (kLeft == 0 and kRight > 0)):
            # ---> When the robot is going on the outside (large path) <--- + ---> I have no idea on why it isn't working <---
            await self.turnLeft((360 - angle) / 3, self.startSpeed, self.endSpeed, kLeft = kLeft, kRight = kRight, stallDetectionIterator = stallDetectionIterator, stallDetectionThreshold = stallDetectionThreshold, accelerationScale = accelerationScale, accelerationMethod = accelerationMethod, addition = 0, divizor = 3, stop = True)
            await self.turnLeft((360 - angle) / 3, self.startSpeed, self.endSpeed, kLeft = kLeft, kRight = kRight, stallDetectionIterator = stallDetectionIterator, stallDetectionThreshold = stallDetectionThreshold, accelerationScale = accelerationScale, accelerationMethod = accelerationMethod, addition = 1, divizor = 3, stop = True)
            await self.turnLeft((360 - angle) / 3, self.startSpeed, self.endSpeed, kLeft = kLeft, kRight = kRight, stallDetectionIterator = stallDetectionIterator, stallDetectionThreshold = stallDetectionThreshold, accelerationScale = accelerationScale, accelerationMethod = accelerationMethod, addition = 2, divizor = 3, stop = stop)
            self.previousFunction = 4; return None
        ...
        print("Turn Right Complete", addition)
...
async def main():
    ...
    # await driveBase.gyroForwards(25, 200, 500, accelerationMethod = SpeedMethods.easeOutIn)
    # await driveBase.gyroBackwards(25, 200, 500, accelerationMethod = SpeedMethods.easeOutIn)
