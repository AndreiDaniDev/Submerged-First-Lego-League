...
class DriveBase(object):
    ...
    async def gyroForwards(self, distance: float, startSpeed: int, endSpeed: int, *, kp: float = 0.5, ki: float = 0.2, kd: float = 0.75, dt: int = 1, integralLimit: int = 250, stallDetectionIterator: int = 25, stallDetectionThreshold: int = 10, accelerationScale: int = 100, accelerationMethod = SpeedMethods.liniar, stop: bool = True) -> None:
        ...
        gs.reset_yaw(self.reachAngle - gs.tilt_angles()[0]); self.reachAngle = 0
    ...
    async def turnLeft(self, angle: int, startSpeed: int, endSpeed: int, *, kLeft: float, kRight: float, accelerationScale: int, accelerationMethod = SpeedMethods.liniar):

        if(self.wasStuck and self.previousFunction == 3 and kLeft == self.kLeft and kRight == self.kRight):
            print("Canceled function"); return None

        gs.reset_yaw(self.reachAngle - gs.tilt_angles()[0])
        ...
        if((kLeft > 0) == (kRight > 0)): print("Error - kLeft and kRight are of the same sign"); return None
...
async def main():
    ...
    await driveBase.gyroBackwards(100, 200, 1000, kp = 0.5, ki = 0.2, kd = 0.75, dt = 25, accelerationMethod = SpeedMethods.easeOutIn, stop = True)
    # await driveBase.gyroForwards(50, 1000, 200, offset = 0, kp = 0.5, ki = 0.2, kd = 0.75, dt = 25, accelerationMethod = SpeedMethods.easeOutIn, stop = True)
runloop.run(main())
